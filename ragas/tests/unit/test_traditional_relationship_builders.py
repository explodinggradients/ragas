...
def generate_test_sets(
    ...
    while current_similar < target_similar and attempts < max_attempts:
        ...
        # solve for needed intersection size I such that
        #   I / (Li + Lj - I) >= min_similarity
        I = math.ceil(min_similarity * (Li + Lj) / (1 + min_similarity))

        # build new similar pair
        shared = {generate_entity() for _ in range(I)}
        Ai = shared | {generate_entity() for _ in range(Li - I)}
        Bj = shared | {generate_entity() for _ in range(Lj - I)}
