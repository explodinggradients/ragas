...
class FactualCorrectness(MetricWithLLM, SingleTurnMetric):
    """
    Evaluates factual correctness using claim decomposition and NLI verification.

    Attributes:
        mode: Evaluation mode ("precision", "recall", "f1")
        beta: F1 score weight (>1 favors recall, <1 favors precision)
        atomicity: Claim decomposition level ("low", "high")
        coverage: Claim decomposition coverage ("low", "high")
    """
    ...
    async def decompose_claims(self, response: str) -> t.List[str]:
        ...
        result = self.llm.generate(
            prompt,
            response_model=ClaimDecompositionOutput,  # type: ignore
        )
