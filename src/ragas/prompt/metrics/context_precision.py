"""Context Precision prompts - V1-identical using exact PydanticPrompt.to_string() output."""
...
def context_precision_prompt(question: str, context: str, answer: str) -> str:
    """
    V1-identical context precision prompt - matches PydanticPrompt.to_string() exactly.

    Args:
        question: The question being asked
        context: The context to evaluate for usefulness
...
def context_precision_with_reference_prompt(
    question: str, context: str, reference: str
) -> str:
    """
    V1-identical context precision prompt for WithReference variant.
    ...
    Returns:
        V1-identical prompt string for the LLM
    """
    # This variant uses "reference" as the "answer" parameter in the prompt
    return context_precision_prompt(question, context, reference)
...
def context_precision_without_reference_prompt(
    ...
    """
    V1-identical context precision prompt for WithoutReference variant.
    ...
    Returns:
        V1-identical prompt string for the LLM
    """
    # This variant uses "response" as the "answer" parameter in the prompt
    return context_precision_prompt(question, context, response)
