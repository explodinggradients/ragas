...
def test_testset_to_evaluation_dataset(eval_sample):
    testset_sample = TestsetSample(eval_sample=eval_sample, synthesizer_name="test")
...
def test_testset_save_load_jsonl(tmpdir, eval_sample):
    ...
    loaded_testset = Testset.from_jsonl(tmpdir / "jsonlfile.jsonl")
...
def test_testset_save_load_hf(tmpdir, eval_sample):
    ...
    loaded_testset = Testset.from_hf_dataset(hf_testset)
    assert loaded_testset == testset
