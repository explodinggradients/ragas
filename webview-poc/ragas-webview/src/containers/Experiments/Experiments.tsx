...
const columnHelper = createColumnHelper<ExperimentFile>();
...
export function Experiments() {
  ...
  useEffect(() => {
    const fetchExperiments = async () => {
      try {
        setLoading(true);
        const data = await backendApiClient.get<ExperimentFile[]>(backendApiRoutes.experiments.list());
        setExperiments(data);
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Failed to fetch experiments';
        showErrorAlert(errorMessage, 'Error loading experiments');
      } finally {
        setLoading(false);
      }
    };
