...
@pytest.mark.skipif(not GDRIVE_AVAILABLE, reason="Google Drive dependencies not available")
class TestGDriveBackendInitialization:
    ...
    def test_service_account_auth_success(self, mock_exists, mock_credentials, mock_build):
        ...
        mock_drive_service.files().get.return_value.execute.return_value = {"id": "test_folder"}
        mock_drive_service.files().list.return_value.execute.side_effect = [
            {"files": []}, {"files": []}  # No existing folders
        ]
        mock_drive_service.files().create.return_value.execute.side_effect = [
            {"id": "datasets_folder"}, {"id": "experiments_folder"}
        ]
    ...
    def test_invalid_folder_id(self, mock_exists, mock_credentials, mock_build):
        ...
        mock_drive_service.files().get.side_effect = Exception("Not found")
...
class TestGDriveBackendOperations:
    ...
    def test_list_datasets(self):
        ...
        backend.drive_service.files().list.return_value.execute.return_value = {
            "files": [
                {"name": "dataset1.gsheet", "mimeType": "application/vnd.google-apps.spreadsheet"},
                {"name": "dataset2.gsheet", "mimeType": "application/vnd.google-apps.spreadsheet"}
            ]
        }
    ...
    def test_list_experiments(self):
        ...
        backend.drive_service.files().list.return_value.execute.return_value = {
            "files": [
                {"name": "experiment1.gsheet"},
                {"name": "experiment2.gsheet"}
            ]
        }
    ...
    def test_complex_data_serialization(self):
        ...
        mock_update = Mock()
        backend.sheets_service.spreadsheets().values().update.return_value.execute = mock_update
        backend.sheets_service.spreadsheets().values().clear.return_value.execute.return_value = {}
...
class TestGDriveBackendIntegration:
    ...
    def test_backend_implements_basebackend(self):
        ...
        required_methods = [
            'load_dataset', 'load_experiment',
            'save_dataset', 'save_experiment',
            'list_datasets', 'list_experiments'
        ]
    ...
    def test_error_without_dependencies(self):
        ...
        with patch('ragas_experimental.backends.gdrive_backend.GDRIVE_AVAILABLE', False):
