...
class Prompt:
    ...
    @classmethod
    def load(cls, path: str, response_model: t.Optional["BaseModel"] = None) -> "Prompt":
        """
        ...
        if data.get("type") != "Prompt":
            raise ValueError(f"File is not a Prompt (found type: {data.get('type', 'unknown')})")
    ...
    def _validate_response_model(self, provided_model: "BaseModel", expected_info: t.Dict) -> None:
        ...
        if (expected_schema.get("properties") != actual_schema.get("properties") or
            expected_schema.get("required") != actual_schema.get("required")):
            warnings.warn(
