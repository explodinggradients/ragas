...
from .backends.base import DatasetBackend
from .typing import SUPPORTED_BACKENDS
import ragas_experimental.typing as rt
from .metric import MetricResult

# Import GDriveBackend with optional import
try:
...
def create_dataset_backend(backend_type: SUPPORTED_BACKENDS, **kwargs):
    ...
    Args:
        backend_type: The type of backend to create (ragas_app, local, or gdrive)
    ...
    # Add GDriveBackend if available
    if GDRIVE_AVAILABLE and GDriveBackend is not None:
        backend_classes["gdrive"] = GDriveBackend
    ...
    if backend_type not in backend_classes:
        available_backends = list(backend_classes.keys())
        raise ValueError(f"Unsupported backend: {backend_type}. Available backends: {available_backends}")
...
class Dataset(t.Generic[BaseModelType]):
    ...
    def __init__(
        ...
        Args:
            ...
            datatable_type: Type of datatable (datasets or experiments)
            ragas_api_client: Required for ragas_app backend
            backend: The storage backend to use (ragas_app, local, or gdrive)
            local_root_dir: Required for local backend
            gdrive_folder_id: Required for gdrive backend - Google Drive folder ID
            gdrive_credentials_path: Optional for gdrive backend - OAuth credentials path
            gdrive_service_account_path: Optional for gdrive backend - Service account path
            gdrive_token_path: Optional for gdrive backend - Token storage path
        """
        ...
        if backend == "ragas_app":
            ...
            if gdrive_folder_id is None:
                raise ValueError("gdrive_folder_id is required for gdrive backend")
            backend_params = {
