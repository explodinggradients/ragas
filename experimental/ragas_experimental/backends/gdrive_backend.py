...
class GDriveBackend(BaseBackend):
    ...
    def _ensure_folder_structure(self):
        ...
        self.experiments_folder_id = self._get_or_create_folder("experiments", self.folder_id)
    ...
    def _get_folder_id_for_type(self, data_type: str) -> str:
        ...
        if data_type == "datasets":
            ...
            raise ValueError(f"Invalid data type: {data_type}. Must be 'datasets' or 'experiments'")
    ...
    def _load_data_from_spreadsheet(self, name: str, data_type: str) -> t.List[t.Dict[str, t.Any]]:
        """Load data from a Google Sheet."""
        if not self._spreadsheet_exists(name, data_type):
            # Use singular form for error message
            singular_type = data_type.rstrip('s') if data_type.endswith('s') else data_type
