...
class BackendRegistry:
    ...
    def register_backend(
        ...
        if not issubclass(backend_class, ProjectBackend):
            raise TypeError(f"Backend class {backend_class} must inherit from ProjectBackend")
        
        # Check for existing registration
        if name in self._backends and not overwrite:
            raise ValueError(f"Backend '{name}' is already registered. Use overwrite=True to replace.")
        ...
        if aliases:
            for alias in aliases:
                if not alias or not isinstance(alias, str):
                    logger.warning(f"Invalid alias '{alias}' for backend '{name}', skipping")
                    continue
    ...
    def list_all_names(self) -> t.Dict[str, t.List[str]]:
        ...
        for primary_name in self._backends.keys():
            aliases = [alias for alias, target in self._aliases.items() if target == primary_name]
    ...
    def discover_backends(self) -> t.Dict[str, t.Type[ProjectBackend]]:
        ...
        logger.info(f"Backend discovery complete. Found {len(self._backends)} backends.")
        ...
        try:
            ...
            for entry_point in entry_points:
                try:
                    ...
                    logger.info(f"Discovered backend from entry point: {entry_point.name}")
                    
                except Exception as e:
                    logger.warning(f"Failed to load backend '{entry_point.name}': {e}")
                    
        except Exception as e:
            logger.debug(f"Entry point discovery failed (this is normal if no plugins installed): {e}")
